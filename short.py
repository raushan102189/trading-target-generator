...
class Short():
    ...
    def shorto(self):
        risk_on_1_share =  self.stoploss - self.current 
        quantity = (self.maxloss / risk_on_1_share)
        entry_price = self.current * quantity
        target = risk_on_1_share *2
        target_price = self.current - target 
        target_percentage = (self.current/ target_price)
        exit_price = target_percentage * entry_price
        stoploss_hit_percentage = self.current / self.stoploss 
        ...
        try_to_save("short",self.current,target,self.stoploss,quantity,net_profit,self.reason)
